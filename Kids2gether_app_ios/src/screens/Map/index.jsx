...
export default function Map() {
  ...
  const safeUserMarkers = Array.isArray(userMarkers)
    ? userMarkers
        .filter((item) => isValidCoordinate(item?.coordinates))
        .map((item) => ({ ...item, coordinates: toCoordinate(item.coordinates) }))
    : [];

  const safeContentMarkers = Array.isArray(contentMarkers)
    ? contentMarkers
        .filter((item) => isValidCoordinate(item?.coordinates))
        .map((item) => ({ ...item, coordinates: toCoordinate(item.coordinates) }))
    : [];
  ...
  const selectedPin = (index) => {
    const data = safeUserMarkers.filter((item) => String(item.id) === String(index));
